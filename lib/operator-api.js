  ...
  return async function operatorApiHandler(req, res) {
    ...
    try {
      ...
      if (error instanceof HttpError && error.status < 500) {
        ...
        const detail = error.details?.currentRevision === undefined ? {} : { currentRevision: error.details.currentRevision };
        sendJson(res, error.status, { error: error.message, code, ...detail });
        return;
      }
