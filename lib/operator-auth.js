...
const OPAQUE_SESSION_TOKEN = /^[A-Za-z0-9_-]{16,256}$/;

export async function findNeonOpaqueSession(token) {
  const rows = await queryNeon`SELECT u.id, u.name, u.email
    FROM neon_auth.session AS s
    JOIN neon_auth."user" AS u ON u.id = s."userId"
    WHERE s.token = ${token} AND s."expiresAt" > now()
    LIMIT 1`;
  const user = rows[0];
  return user ? { user, provider: "neon" } : null;
}

export function createNeonIdentityProvider({
  jwksUrl = process.env.NEON_AUTH_JWKS_URL,
  issuer = process.env.NEON_AUTH_BASE_URL,
  findOpaqueSession = findNeonOpaqueSession,
  verifyJwt = jwtVerify
} = {}) {
  ...
  const jwtOptions = issuer ? { issuer: new URL(issuer).origin } : {};
  return createIdentityProvider({ async verifySession(req) {
    const match = /^Bearer\s+([^\s]+)$/i.exec(String(header(req, "authorization") ?? ""));
    if (!match) return null;
    const token = match[1];
    if (!token.includes(".")) {
      if (!OPAQUE_SESSION_TOKEN.test(token)) return null;
      try { return await findOpaqueSession(token); }
      catch (error) {
    ...
    try {
      const { payload } = await verifyJwt(token, jwks, jwtOptions);
