...
export const findOwnedDraft = find;
...
export const completeDraft = complete;
export async function claimNotification(row, payload) {
  ...
  return claimed[0]?.notification_payload ?? null;
}
export async function finishNotification(row, state) {
...
export async function notify(row) {
  ...
  const claimed = await claimNotification(row, payload);
  if (!claimed) return;
  let state='failed';
  try { await sendInquiryEmail(row.id,claimed); state='sent'; } catch { /* Safe state only. */ }
  await finishNotification(row, state);
}
