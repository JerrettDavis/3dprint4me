...
export function createNeonWorkRepository({ query = queryNeon } = {}) {
  return {
    ...
    async getHomeAssistantSnapshot() {
      const rows = await query`WITH active AS (
          SELECT w.id, w.status, w.priority, w.acknowledged_at, w.created_at, w.target_date,
                 sr.service, sr.project_title, sr.submitted_at
          FROM work_items w JOIN service_requests sr ON sr.id = w.request_id
          WHERE w.status NOT IN ('completed', 'declined', 'cancelled')
        ), counts AS (
          ...
          SELECT id, status, priority, acknowledged_at, target_date, service, project_title, submitted_at,
                 created_at, CASE priority WHEN 'urgent' THEN 0 WHEN 'high' THEN 1 WHEN 'normal' THEN 2 ELSE 3 END AS priority_rank
          FROM active ORDER BY priority_rank, created_at, id LIMIT 20
        )
        SELECT counts.active, counts.unacknowledged, counts.urgent, counts.waiting_customer,
               (SELECT id FROM work_items ORDER BY created_at DESC, id DESC LIMIT 1) AS latest_created_id,
               COALESCE((SELECT jsonb_agg(jsonb_build_object(
                 'id', id, 'status', status, 'service', service, 'projectTitle', project_title,
                 'priority', priority, 'acknowledged', acknowledged_at IS NOT NULL,
                 'submittedAt', submitted_at, 'targetDate', target_date
               ) ORDER BY priority_rank, created_at, id) FROM limited), '[]'::jsonb) AS items
        FROM counts`;
      const row = rows[0];
      return {
        counts: {
          ...
          waitingCustomer: Number(row?.waiting_customer ?? 0)
        },
        latestCreatedId: row?.latest_created_id ?? null,
        items: (row?.items ?? []).map(item => ({
          id: item.id, status: item.status, service: item.service, projectTitle: item.projectTitle,
          priority: item.priority, acknowledged: Boolean(item.acknowledged),
          submittedAt: new Date(item.submittedAt).toISOString(), targetDate: iso(item.targetDate)?.slice?.(0, 10) ?? null
        }))
      };
    },
    async getWork(id) {
...
export const createNeonWorkStore = createNeonWorkRepository;
const defaultStore = createNeonWorkRepository();
