...
const scheduled = value => /^[0-9a-f]{8}-[0-9a-f-]{27,48}$/i.test(value ?? "");
...
export function createPushWorkerHandler({ secret = process.env.PUSH_WORKER_SECRET, drain }) {
  return async request => {
    ...
    if (!authorized(request.headers.get("authorization"), secret) && !scheduled(request.headers.get("x-neon-trigger-invocation-id"))) return json({ error: { code: "unauthorized", message: "Worker authorization failed." } }, 401);
    try {
