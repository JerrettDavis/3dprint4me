...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname === "/config.json") { const body = JSON.stringify({ apiBase }); res.writeHead(200, { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "Content-Length": Buffer.byteLength(body) }); res.end(body); return; }
